...
yteste <- dat_test$BW # Selecting the weigth as phenotypic measure for the test set
ytreino <- dat_train$BW # Selecting the weigth as phenotypic measure for the train set
...
seq_h2 = rep(weight_heritability$h2, 6)
...
p1 + geom_point(aes(color=factor(Model)), size = 4) + labs(x="Heritability",y="Correlation with test data") + geom_point(colour = "grey90", size = 1.5) + ggtitle("Weight")
#  +geom_text(aes(label=cor.tst),hjust=0, vjust=0)
############################################################
...
p2 + geom_point(aes(color=factor(Model)), size = 4) + geom_point(colour = "grey90", size = 1.5) + ggtitle("Weight") +labs(x="Heritability",y="Correlation with train data")  
###############################################

par(mfrow=c(2,3))
plot(fmRBB$yHat, ytreino, xlab="Weight",
     ylab="Estimated phenotype", cex=.8, bty="L", main= 'Bayes B')

plot(fmRBA$yHat, ytreino, xlab="Weight",
     ylab="Estimated phenotype", cex=.8, bty="L", main= 'Bayes A')

plot(fmRBRR$yHat, ytreino, xlab="Weight",
     ylab="Estimated phenotype", cex=.8, bty="L", main= 'Bayes RR')

plot(fmRBC$yHat, ytreino, xlab="Weight",
     ylab="Estimated phenotype", cex=.8, bty="L", main= 'Bayes C')

plot(fmRBL$yHat, ytreino, xlab="Weight",
     ylab="Estimated phenotype", cex=.8, bty="L", main= 'Bayes Lasso')

plot(GEBVRRBlup2%*%geno_test_noNA, ytreino, xlab="Weight",
     ylab="Estimated phenotype", cex=.8, bty="L", main= 'RRBlup')
...
tmp <- range(c(ytreino, fmRBB$yHat))
plot(GEBVBB, yteste, ylab='Phenotype', xlab='Predicted Genomic Value')
